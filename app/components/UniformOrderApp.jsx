...
export default function App() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setSuccess(true); // show success even if API not configured
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div style={{
      minHeight:"100vh",
      background:"linear-gradient(160deg,#0F3D68 0%,#1a5490 40%,#0d2d4e 100%)",
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .btn-secondary { background:rgba(255,255,255,0.1); color:#fff; border:1.5px solid rgba(255,255,255,0.2); padding:14px 24px; border-radius:12px; font-size:15px; font-family:'Sarabun',sans-serif; cursor:pointer; transition:background 0.2s; }
        .btn-secondary:hover { background:rgba(255,255,255,0.18); }
        .uniform-card { border-radius:16px; border:2px solid rgba(255,255,255,0.12); background:rgba(255,255,255,0.06); cursor:pointer; transition:all 0.25s; overflow:hidden; }
        .uniform-card:hover { border-color:rgba(212,160,23,0.5); background:rgba(255,255,255,0.1); transform:translateY(-3px); }
        .uniform-card.selected { border-color:#D4A017; background:rgba(212,160,23,0.12); box-shadow:0 0 0 1px #D4A017; }
        .pay-card { border-radius:14px; border:2px solid rgba(255,255,255,0.12); background:rgba(255,255,255,0.06); cursor:pointer; padding:18px; transition:all 0.2s; }
        .pay-card:hover { border-color:rgba(212,160,23,0.4); }
        ...
        .step-dot.active { background:#D4A017; color:#0F3D68; box-shadow:0 0 0 4px rgba(212,160,23,0.25); }
        ...
        .glass { background:rgba(255,255,255,0.07); backdrop-filter:blur(16px); border:1px solid rgba(255,255,255,0.12); border-radius:20px; padding:24px; }
        ...
        .upload-zone { border:2px dashed rgba(255,255,255,0.2); border-radius:14px; padding:32px 20px; text-align:center; cursor:pointer; transition:all 0.2s; }
        .upload-zone:hover { border-color:#D4A017; background:rgba(212,160,23,0.05); }
        @keyframes pulse { 0%,100%{transform:scale(1)}50%{transform:scale(1.04)} }
        @keyframes fadeIn { from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)} }
        .fade-in { animation:fadeIn 0.4s ease forwards; }
        .gold-text { color:#D4A017; }
        .light-blue { color:#DFF4F8; }
      `}</style>

      {/* Sticky Header */}
      <div style={{
        position:"sticky", top:0, zIndex:100,
        background:"rgba(15,61,104,0.95)", backdropFilter:"blur(20px)",
        borderBottom:"1px solid rgba(255,255,255,0.08)",
        padding:"12px 16px"
      }}>
        <div style={{maxWidth:600, margin:"0 auto", display:"flex", alignItems:"center", gap:12}}>
          <img src={SCHOOL_LOGO} alt="โลโก้โรงเรียน" style={{width:44,height:44,objectFit:"contain",borderRadius:6}} onError={e=>e.target.style.display="none"} />
          <div>
            <div style={{color:"#D4A017",fontWeight:700,fontSize:13,lineHeight:1.3}}>ระบบสั่งจองชุดพิธีการ</div>
      ...
      <div style={{maxWidth:600, margin:"0 auto", padding:"0 16px"}}>
        ...
        <div style={{textAlign:"center", padding:"36px 0 24px"}} className="fade-in">
          <img src={SCHOOL_LOGO} alt="โลโก้" style={{width:90,height:90,objectFit:"contain",marginBottom:16,filter:"drop-shadow(0 4px 16px rgba(212,160,23,0.3))"}} onError={e=>e.target.style.display="none"} />
          <h1 style={{color:"#DFF4F8",fontSize:22,fontWeight:700,lineHeight:1.5,margin:"0 0 8px"}}>
            ระบบสั่งจองชุดพิธีการ
          </h1>
          <div style={{color:"#D4A017",fontSize:16,fontWeight:600,marginBottom:6}}>
          ...
          <div style={{
            ...
            borderRadius:12, padding:"10px 20px", marginTop:16,
            color:"rgba(255,255,255,0.65)", fontSize:13, lineHeight:1.7
          }}>
            เครื่องแบบพิธีการ สัญลักษณ์แห่งความสง่างาม<br/>
            หลอมรวมคุณค่าแห่งเกียรติยศและความภาคภูมิใจ
          </div>
        </div>
        ...
        <div style={{display:"flex", alignItems:"center", justifyContent:"center", gap:0, marginBottom:28}}>
          {steps.map((s,i) => (
            <div key={i} style={{display:"flex", alignItems:"center"}}>
              ...
              {i < steps.length-1 && (
                <div style={{width:28,height:2,background:i<step?"rgba(212,160,23,0.5)":"rgba(255,255,255,0.1)",marginBottom:20,marginLeft:2,marginRight:2}} />
              )}
            </div>
          ))}
        </div>
            ...
            <div className="glass">
                ...
                {[
                  ...
                  <div key={u.id} className={`uniform-card ${form.uniformType===u.id?"selected":""}`} onClick={()=>set("uniformType",u.id)}>
                    <div style={{position:"relative",paddingTop:"120%",overflow:"hidden"}}>
                      <img src={u.img} alt={u.label} style={{position:"absolute",top:0,left:0,width:"100%",height:"100%",objectFit:"cover",objectPosition:"top"}} onError={e=>{e.target.parentElement.style.display="flex";e.target.parentElement.style.alignItems="center";e.target.parentElement.style.justifyContent="center";e.target.parentElement.style.paddingTop="0";e.target.parentElement.style.height="140px";e.target.style.display="none";e.target.parentElement.innerHTML=`<span style="font-size:48px">${u.icon}</span>`;}} />
                      {form.uniformType===u.id && (
                        <div style={{position:"absolute",top:8,right:8,background:"#D4A017",borderRadius:"50%",width:24,height:24,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#0F3D68"}}>✓</div>
                      )}
                    </div>
              ...
              {form.uniformType && (
                <div style={{marginTop:16,background:"rgba(212,160,23,0.1)",borderRadius:12,padding:"12px 16px",border:"1px solid rgba(212,160,23,0.25)"}}>
                  ...
                  <div style={{color:"#D4A017",fontWeight:700,fontSize:16}}>
                    {form.uniformType==="male"?"ชุดพิธีการชาย":"ชุดพิธีการหญิง"} — {fmtPrice(price)} บาท
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="fade-in">
            <div className="glass" style={{marginBottom:16}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                <div className={`pay-card ${form.paymentType==="deposit"?"selected":""}`} onClick={()=>set("paymentType","deposit")}>
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    <div style={{width:20,height:20,borderRadius:"50%",border:`2px solid ${form.paymentType==="deposit"?"#D4A017":"rgba(255,255,255,0.3)"}`,display:"flex",alignItems:"center",justifyContent:"center"}}>
                      {form.paymentType==="deposit" && <div style={{width:10,height:10,borderRadius:"50%",background:"#D4A017"}} />}
                    </div>
                    <div style={{flex:1}}>
                      <div style={{color:"#DFF4F8",fontWeight:600,fontSize:15}}>มัดจำ 1,000 บาท</div>
                      <div style={{color:"rgba(255,255,255,0.5)",fontSize:12,marginTop:2}}>
                        ยอดคงเหลือ {fmtPrice(price - DEPOSIT)} บาท
                      </div>
                    </div>
                    <div style={{color:"#D4A017",fontWeight:700,fontSize:18}}>{fmtPrice(DEPOSIT)}</div>
                  </div>
                </div>
                <div className={`pay-card ${form.paymentType==="full"?"selected":""}`} onClick={()=>set("paymentType","full")}>
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    <div style={{width:20,height:20,borderRadius:"50%",border:`2px solid ${form.paymentType==="full"?"#D4A017":"rgba(255,255,255,0.3)"}`,display:"flex",alignItems:"center",justifyContent:"center"}}>
                      {form.paymentType==="full" && <div style={{width:10,height:10,borderRadius:"50%",background:"#D4A017"}} />}
                    </div>
                    <div style={{flex:1}}>
                      <div style={{color:"#DFF4F8",fontWeight:600,fontSize:15}}>ชำระเต็มจำนวน</div>
                      <div style={{color:"rgba(255,255,255,0.5)",fontSize:12,marginTop:2}}>ไม่มียอดค้างชำระ</div>
                    </div>
                    <div style={{color:"#D4A017",fontWeight:700,fontSize:18}}>{fmtPrice(price)}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="glass" style={{marginBottom:16,textAlign:"center"}}>
              ...
              <div style={{
                ...
                <img src={QR_IMAGE} alt="QR PromptPay" style={{width:220,height:"auto",display:"block"}} onError={e=>{e.target.parentElement.innerHTML='<div style="width:220px;height:220px;background:#f3f4f6;display:flex;align-items:center;justify-content:center;border-radius:8px;color:#666;font-size:13px">QR Code</div>';}} />
              </div>
              ...
              <div style={{display:"flex",gap:8,marginTop:12,justifyContent:"center"}}>
                <button className="btn-secondary" style={{padding:"8px 16px",fontSize:13}} onClick={()=>setQrOpen(true)}>
                  🔍 ดูภาพเต็ม
                </button>
            ...
            <div className="glass" style={{marginBottom:16}}>
              ...
              {!form.slipPreview ? (
                ...
                <div style={{textAlign:"center"}}>
                  ...
                  <button className="btn-secondary" style={{marginTop:8,padding:"8px 16px",fontSize:13}} onClick={()=>{set("slipFile",null);set("slipPreview",null);set("slipName","");}}>
                    เปลี่ยนไฟล์
                  </button>
                </div>
              )}
        ...
        {step === 3 && (
          <div className="fade-in">
            ...
            <button className="btn-primary" onClick={submit} disabled={submitting} style={{
              fontSize:17, padding:"18px 32px",
              background:submitting?"rgba(212,160,23,0.5)":"linear-gradient(135deg,#D4A017,#e8b520)"
            }}>
      ...
      {qrOpen && (
        <div onClick={()=>setQrOpen(false)} style={{
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:20,padding:24,maxWidth:380,width:"100%",textAlign:"center"}}>
            <img src={QR_IMAGE} alt="QR PromptPay" style={{width:"100%",height:"auto",borderRadius:12}} onError={e=>{e.target.src="";}} />
...
function SuccessModal({name, uniformType}) {
  return (
    <div style={{
      minHeight:"100vh",
      background:"linear-gradient(160deg,#0F3D68,#1a5490)",
      display:"flex",alignItems:"center",justifyContent:"center",
      padding:24,fontFamily:"'Sarabun',sans-serif"
    }}>
      <style>@import url('https://fonts.googleapis.com/css2?family=Sarabun:wght@400;600;700&display=swap');</style>
      <div style={{
        background:"rgba(255,255,255,0.07)",backdropFilter:"blur(20px)",
        border:"1px solid rgba(255,255,255,0.15)",borderRadius:24,padding:40,
        textAlign:"center",maxWidth:400,width:"100%",
        animation:"fadeIn 0.5s ease"
      }}>
        <div style={{fontSize:72,marginBottom:16,animation:"pulse 1.5s ease infinite"}}>🎉</div>
        ...
        <div style={{color:"rgba(255,255,255,0.6)",fontSize:13,lineHeight:1.7,marginBottom:24}}>
          การสั่งจอง{uniformType==="male"?"ชุดพิธีการชาย":"ชุดพิธีการหญิง"}ของคุณ<br/>
          ได้รับการบันทึกเรียบร้อยแล้ว<br/>
          ทางโรงเรียนจะติดต่อกลับเพื่อยืนยัน
        </div>
        <div style={{
        ...
        <button onClick={()=>window.location.reload()} style={{
          ...
          color:"#0F3D68",border:"none",padding:"14px 32px",
          borderRadius:12,fontSize:15,fontWeight:700,cursor:"pointer",
          fontFamily:"'Sarabun',sans-serif",width:"100%"
        }}>สั่งจองใหม่</button>
      </div>
    </div>
  );
